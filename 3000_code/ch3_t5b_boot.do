...
capture log close
log using "3000_code/SMCL_logs/ch3_t5b_boot", smcl replace

* list of predictors for table 
local predictors = "border_pop_share freq_border_pop_share avg_border_pop_share med_border_pop_share"
...
save "2000_data/toboot_t5b", replace
...
return scalar b_avg_border_pop_share = .
return scalar b_med_border_pop_share = .
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_s`seed'_t5b, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_s`seed'_t5b
...
keep statefip state_fips obsid elast border_pop_share freq_border_pop_share avg_border_pop_share med_border_pop_share licensed_drivers 
...
reg elast avg_border_pop_share [aweight=licensed_drivers], robust 
return scalar b_avg_border_pop_share = _b[avg_border_pop_share]

reg elast med_border_pop_share [aweight=licensed_drivers], robust 
return scalar b_med_border_pop_share = _b[med_border_pop_share]
...
****Check program
bootstrap b_border_pop_share=r(b_border_pop_share) b_freq_border_pop_share=r(b_freq_border_pop_share) b_avg_border_pop_share=r(b_avg_border_pop_share) b_med_border_pop_share=r(b_med_border_pop_share), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_s`seed'_t5b.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_s`seed'_t5b.dta", clear
tabstat b_border_pop_share, stat(sd)
tabstat b_freq_border_pop_share, stat(sd)
tabstat b_avg_border_pop_share, stat(sd)
tabstat b_med_border_pop_share, stat(sd)
