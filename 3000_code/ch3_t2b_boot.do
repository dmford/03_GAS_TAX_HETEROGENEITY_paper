...
capture log close
log using "3000_code/SMCL_logs/ch3_t2b_boot", smcl replace

* list of predictors for table 
local predictors = "land_area weather_index flatness_index"
...
save "2000_data/toboot_t2b", replace
...
return scalar b_flatness_index = .
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_s`seed'_t2b, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_s`seed'_t2b
...
keep statefip state_fips obsid elast land_area weather_index flatness_index licensed_drivers 
...
reg elast flatness_index [aweight=licensed_drivers], robust 
return scalar b_flatness_index = _b[flatness_index]
...
****Check program
bootstrap b_land_area=r(b_land_area) b_weather_index=r(b_weather_index) b_flatness_index=r(b_flatness_index), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_s`seed'_t2b.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_s`seed'_t2b.dta", clear
tabstat b_land_area, stat(sd)
tabstat b_weather_index, stat(sd)
tabstat b_flatness_index, stat(sd)
