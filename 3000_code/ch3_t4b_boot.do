...
capture log close
log using "3000_code/SMCL_logs/ch3_t4b_boot", smcl replace
...
save "2000_data/toboot_t4b", replace
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_s`seed'_t4b, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_s`seed'_t4b
...
****Check program
bootstrap b_state_pop=r(b_state_pop) b_pop_density=r(b_pop_density) b_border_pop_share=r(b_border_pop_share) b_licensure=r(b_licensure), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_s`seed'_t4b.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_s`seed'_t4b.dta", clear
