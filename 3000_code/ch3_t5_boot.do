...
capture log close
log using "3000_code/SMCL_logs/ch3_t5_boot", smcl replace
...
save "2000_data/toboot_t5", replace
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_s`seed'_t5, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_s`seed'_t5
