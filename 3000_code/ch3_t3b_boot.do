...
capture log close
log using "3000_code/SMCL_logs/ch3_t3b_boot", smcl replace

* list of predictors for table 
local predictors = "urban_mileage total_mileage urban_roadshare pub_tran_average"
...
save "2000_data/toboot_t3b", replace
...
return scalar b_urban_mileage = .
return scalar b_total_mileage = .
return scalar b_urban_roadshare = .
return scalar b_pub_tran_average = .
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_s`seed'_t3b, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_s`seed'_t3b
...
keep statefip state_fips obsid elast urban_mileage total_mileage urban_roadshare pub_tran_average licensed_drivers 
duplicates drop statefip, force

*local predictors = "urban_mileage total_mileage urban_roadshare pub_tran_average"

* regressions 
reg elast urban_mileage [aweight=licensed_drivers], robust 
return scalar b_urban_mileage = _b[urban_mileage]

reg elast total_mileage [aweight=licensed_drivers], robust 
return scalar b_total_mileage = _b[total_mileage]

reg elast urban_roadshare [aweight=licensed_drivers], robust 
return scalar b_urban_roadshare = _b[urban_roadshare]

reg elast pub_tran_average [aweight=licensed_drivers], robust 
return scalar b_pub_tran_average = _b[pub_tran_average]
...
****Check program
bootstrap b_urban_mileage=r(b_urban_mileage) b_total_mileage=r(b_total_mileage) b_urban_roadshare=r(b_urban_roadshare) b_pub_tran_average=r(b_pub_tran_average), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_s`seed'_t3b.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_s`seed'_t3b.dta", clear
tabstat b_urban_mileage, stat(sd)
tabstat b_total_mileage, stat(sd)
tabstat b_urban_roadshare, stat(sd)
tabstat b_pub_tran_average, stat(sd)
