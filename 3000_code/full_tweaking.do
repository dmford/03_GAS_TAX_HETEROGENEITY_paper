...
* before dropping duplicates, need to "prune" so that it drops correctly 
	* simply sorting so that zeroes and blanks (from counties without borders) are not first (duplicate drop goes in sort order)
gen neg_border_pop_share = border_pop_share * -1
gen neg_freq_border_pop_share = freq_border_pop_share * -1
sort state_fips neg_border_pop_share neg_freq_border_pop_share
drop neg_*
