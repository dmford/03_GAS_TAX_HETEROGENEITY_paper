...
capture log close
log using "3000_code/SMCL_logs/t2_s2_boot", smcl replace
...
save "2000_data/toboot_t2_s2", replace
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_t2_s2, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_t2_s2
...
****Check program
bootstrap b_land_area=r(b_land_area) b_weather_index=r(b_weather_index) b_flatness_index=r(b_flatness_index), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_t2_s2.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_t2_s2.dta", clear
