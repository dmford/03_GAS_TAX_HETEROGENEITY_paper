...
capture log close
log using "3000_code/SMCL_logs/t3_s9_boot", smcl replace
...
save "2000_data/toboot_t3_s9", replace
...
statsby temp_first_fstat_`m'_`i'=e(F), by(statefip) saving(statsby`i'_`m'_t3_s9, replace): reg x_tilde`i'_`m' `pc_ivlist'

sort statefip
merge m:1 statefip using statsby`i'_`m'_t3_s9
...
****Check program
bootstrap b_urban_mileage=r(b_urban_mileage) b_total_mileage=r(b_total_mileage) b_urban_roadshare=r(b_urban_roadshare) b_pub_tran_average=r(b_pub_tran_average), reps(`bootnum') seed(`seed') cluster(statefip) idcluster(state_fips) saving("2000_data/ch3_boot_t3_s9.dta", every(10) replace): ch3_boot

timer off 1
timer list

use "2000_data/ch3_boot_t3_s9.dta", clear
